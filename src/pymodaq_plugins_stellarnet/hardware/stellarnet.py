...
class StellarNet(object):
        ...
        if scans_to_avg > 1:
            ...
            for i in range(2, scans_to_avg + 1):
                ...
                for j in range(len(summed)):
                    summed[j] = int(
                        summed[j] * ((i - 1) / float(i)) + data[j] / float(i)
                    )
            data = summed
    ...
    def get_stored_bytes(self, address):
        ...
        return self._device.ctrl_transfer(StellarNet._IN_DEVICE, 0xB5, 0, 0, 0x21)[1:]
    ...
    def _init_config(self):
        ...
        self._config = dict()
        self.set_config(
            int_time=100, x_timing=3, x_smooth=0, scans_to_avg=1, temp_comp=0
        )

        try:
            # Try to fetch and parse lambda coefficients
            coeffs = [
                float(self.get_stored_string(addr).split()[0])
                for addr in [
                    StellarNet.COEFF_C1_ADDR,
                    StellarNet.COEFF_C2_ADDR,
                    StellarNet.COEFF_C3_ADDR,
                    StellarNet.COEFF_C4_ADDR,
                ]
            ]
        except:
    ...
    def _set_device_timing(self):
        ...
        self._device.ctrl_transfer(
            StellarNet._OUT_DEVICE, 0xB4, 0, 0, [msb, lsb, xt, msd]
        )
    ...
    with open(os.path.join(__location__, filename), "r") as fh:
        for line in fh:
            ...
            if count:
                ...
                payload = bytearray.fromhex(line[9 : 9 + count * 2])
                device.ctrl_transfer(StellarNet._OUT_DEVICE, 0xA0, address, 0, payload)
...
def find_devices():
    ...
    while True:
        ...
        if len(list(devices)) >= total_count:
            break
...
def _plot_spectrum(args):
    ...
    print(
        ("Plot: x-axis: {}, y-axis: {}-{} counts".format(x_label, min_value, max_value))
    )
    ...
    print(
        (
            '{{"data":[{}],"timestamp":{}}}'.format(
                ",".join(map(str, device.read_spectrum())), int(time.time() * 1000)
            )
        )
    )
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Exercise StellarNet spectrometer.",
    ...
    param_parser = argparse.ArgumentParser(
        description="spectrometer parameters", add_help=False
    )
    param_parser.add_argument(
        "-i", "--int_time", type=int, help="integration time (ms)"
    )
    ...
    param_parser.add_argument(
        "-p", "--temp_comp", type=int, help="temperature compensation (not implemented)"
    )

    info_parser = subparsers.add_parser(
        "info", help="print device information (default)"
    )
    ...
    plot_parser = subparsers.add_parser(
        "plot", parents=[param_parser], help="plot spectrum"
    )
    plot_parser.set_defaults(func=_plot_spectrum)

    perf_parser = subparsers.add_parser(
        "perf", parents=[param_parser], help="run performance test"
    )
    perf_parser.add_argument(
        "-r", "--repeats", type=int, default=1, help="scan repetition count"
    )
    perf_parser.set_defaults(func=_run_perf_test)

    data_parser = subparsers.add_parser(
        "data", parents=[param_parser], help="print json-formatted scan data"
    )
